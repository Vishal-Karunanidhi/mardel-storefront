...
const PromoComponent = (promoProps: PromoComponentProps): JSX.Element => {
  ...
  const { showCartBagIcon = true, showShareIcon = false } = promoProps;
  const { defaultHost, endpoint, name } = image;
  const imageSrc = `https://${defaultHost}/i/${endpoint}/${name}?fmt=auto`;
  ...
  const iconButton = (imgSrc: string): JSX.Element => (
    <button className={styles.cartButton} onClick={() => window.open(link, '_self')}>
    ...
    >
      <>
        ...
        >
          <CardMedia
            component="img"
            image={imageUrlQuery(imageSrc, 350)}
            alt={imageAltText}
      ...
      <div className={styles.cardActions}>
        <div className={styles.cardActionsButtons}>
          {showCartBagIcon && iconButton('/cartBag.svg')}
...
export default function SecondaryPromotion3x(props: SecondaryPromoComponentProps): JSX.Element {
